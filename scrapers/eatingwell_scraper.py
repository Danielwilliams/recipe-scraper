...
def _extract_recipe_info(self, html_content, url):
    ...
    def _extract_recipe_info(self, html_content, url):
        ...
        try:
            ...
            # Try alternate ingredient selectors if needed
            if not ingredients:
                ingredient_items = soup.select('.ingredients-item-name')
                for item in ingredient_items:
            ...
            # Try alternate instruction selectors if needed
            if not instructions:
                instruction_items = soup.select('.recipe-directions__list--item')
                for item in instruction_items:
                    step = item.text.strip()
                    if step:
                        instructions.append(step)
